...
int main() {
    ...
    float mediaTotala = (media1 + media2) / 2;
    printf("Media aritmetica a celor doua vectoruri: %.2f\n", mediaTotala);

    int vector3[n], cnt = 0;
    for (int i = 0; i < n; i++) {
        if (vector1[i] > mediaTotala) {
            vector3[cnt++] = vector1[i];
        }
        if (vector2[i] > mediaTotala) {
            vector3[cnt++] = vector2[i];
        }
    }

    printf("Vectorul cu elementele mai mari decat media: \n");
    for (int i = 0; i < cnt; i++) {
        printf("%d ", vector3[i]);
    }
    printf("\n");
